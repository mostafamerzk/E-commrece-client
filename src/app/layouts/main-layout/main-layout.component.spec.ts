...
describe('MainLayoutComponent', () => {
  ...
  beforeEach(async () => {
    ...
    (mockAuthService as any).isLoggedIn = signal(false);
    (mockAuthService as any).currentUser = signal<User | null>(null);

    mockCartService = jasmine.createSpyObj('CartService', []);
    (mockCartService as any).itemCount = signal(0);

    mockWishlistService = jasmine.createSpyObj('WishlistService', []);
    (mockWishlistService as any).itemCount = signal(0);
  ...
  it('should show cart item count from CartService.itemCount signal', () => {
    ...
    (mockCartService.itemCount as any).set(5);
    fixture.detectChanges();
  ...
  it('should show wishlist item count from WishlistService.itemCount signal', () => {
    /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
    (mockWishlistService.itemCount as any).set(3);
    fixture.detectChanges();
  ...
  it('should show Login/Register links when isLoggedIn() is false', () => {
    /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
    (mockAuthService.isLoggedIn as any).set(false);
    fixture.detectChanges();
  ...
  it('should show user name and logout option when isLoggedIn() is true', () => {
    /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
    (mockAuthService.isLoggedIn as any).set(true);
    /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
